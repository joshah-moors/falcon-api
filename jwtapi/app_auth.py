...
class RefreshTokenV2:
    ...
    def on_post(self, req, resp):
        #
        #    V2 Look for refresh token as a cookie
        #
        if req.get_cookie_values('refreshToken') is None:
            resp.status = falcon.HTTP_401
            return

        refresh_token = req.get_cookie_values('refreshToken')[0]
